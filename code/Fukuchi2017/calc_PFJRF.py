...
# Set the list of speed conditions to analyse
...
def calculate_pfj_loads(participant):
    ...
    pfjrf = {method: {speed: {'time': np.zeros(101), 'pfjrf': np.zeros(101)} for speed in speed_list} for method in plot_col.keys()}
    pfjs = {method: {speed: {'time': np.zeros(101), 'pfjs': np.zeros(101)}  for speed in speed_list} for method in plot_col.keys()}
    qf = {method: {speed: {'time': np.zeros(101), 'qf': np.zeros(101)} for speed in speed_list} for method in plot_col.keys()}
    computation_time = {method: {speed: np.zeros(1) for speed in speed_list} for method in plot_col.keys()}

    # Get participant mass
    mass_kg = participantInfo.loc[participantInfo['FileName'] == participant + 'static.c3d',]['Mass'].values[0]
    ...
    for speed in speed_list:
        ...
        data_folder = os.path.join('..', '..', 'simulations', dataset, participant, speed, 'inverse_dynamics')
        ...
        # Store in dictionary
        qf['inverse_dynamics'][speed]['qf'] = qf_id
        qf['inverse_dynamics'][speed]['time'] = t_norm

        # Calculate PFJRF
        pfjrf_id = k * qf_id

        # Store in dictionary
        pfjrf['inverse_dynamics'][speed]['pfjrf'] = pfjrf_id
        pfjrf['inverse_dynamics'][speed]['time'] = t_norm
        ...
        # Store in dictionary
        pfjs['inverse_dynamics'][speed]['pfjs'] = pfjs_id
        pfjs['inverse_dynamics'][speed]['time'] = t_norm
        ...
        with open(os.path.join(
                data_folder, f'{participant}run{speed}_inverse-dynamics_complex_computation-time.pkl'), 'rb') as pkl_file:
            id_computation = pickle.load(pkl_file)

        # Store in dictionary
        computation_time['inverse_dynamics'][speed][0] = id_computation['time_s']
        ...
        # Set output path for ease of use
        data_folder = os.path.join('..', '..', 'simulations', dataset, participant, speed, 'static_optimisation')
        ...
        states = osim.TimeSeriesTable(os.path.join(
            data_folder, f'{participant}run{speed}_states.sto'))
        muscle_forces = osim.TimeSeriesTable(os.path.join(
            data_folder, f'{participant}run{speed}_StaticOptimization_Complex_force.sto'))
        ...
        # Store in dictionary
        qf['static_optimisation'][speed]['qf'] = qf_so
        qf['static_optimisation'][speed]['time'] = t_norm

        # Calculate PFJRF
        pfjrf_so = k * qf_so

        # Store in dictionary
        pfjrf['static_optimisation'][speed]['pfjrf'] = pfjrf_so
        pfjrf['static_optimisation'][speed]['time'] = t_norm
        ...
        # Store in dictionary
        pfjs['static_optimisation'][speed]['pfjs'] = pfjs_so
        pfjs['static_optimisation'][speed]['time'] = t_norm
        ...
        # Load in computation time data
        with open(os.path.join(
                data_folder, f'{participant}run{speed}_static-optimisation_complex_computation-time.pkl'), 'rb') as pkl_file:
            so_computation = pickle.load(pkl_file)

        # Store in dictionary
        computation_time['static_optimisation'][speed][0] = so_computation['time_s']
        ...
        # Set output path for ease of use
        data_folder = os.path.join('..', '..', 'simulations', dataset, participant, speed, 'dynamic_optimisation')
        ...
        states = osim.MocoTrajectory(os.path.join(
            data_folder, f'{participant}run{speed}_dynamic-optimisation_complex_solution.sto')).exportToStatesTable()
        muscle_forces = osim.TimeSeriesTable(os.path.join(
        ...
        # Store in dictionary
        qf['dynamic_optimisation'][speed]['qf'] = qf_do
        qf['dynamic_optimisation'][speed]['time'] = t_norm

        # Calculate PFJRF
        pfjrf_do = k * qf_do

        # Store in dictionary
        pfjrf['dynamic_optimisation'][speed]['pfjrf'] = pfjrf_do
        pfjrf['dynamic_optimisation'][speed]['time'] = t_norm
        ...
        # Store in dictionary
        pfjs['dynamic_optimisation'][speed]['pfjs'] = pfjs_do
        pfjs['dynamic_optimisation'][speed]['time'] = t_norm
        ...
        # Load in computation time data
        with open(os.path.join(
                data_folder, f'{participant}run{speed}_dynamic-optimisation_computation-time.pkl'), 'rb') as pkl_file:
            do_computation = pickle.load(pkl_file)

        # Store in dictionary
        computation_time['dynamic_optimisation'][speed][0] = do_computation['time_s']
    ...
    for plot_var in ['qf', 'pfjrf', 'pfjs']:
        ...
        fig, ax = plt.subplots(nrows=1, ncols=3, figsize=(14, 5), sharex=True, sharey=True)
        plt.subplots_adjust(left=0.05, right=0.95, top=0.85, bottom=0.125,
                            wspace = 0.1)

        # Loop through speeds plot data
        for speed in speed_list:
            # Set axis to plot on
            plot_ax = ax.flatten()[speed_list.index(speed)]
            # Add title
            plot_ax.set_title(f'{speed[1]}.{speed[2]} m\u00b7s$^{{-1}}$', fontsize=14, fontweight='bold')
            # Loop through methods
            for method in plot_col.keys():
                # Normalise quads force and pfjrf to body weight
                if plot_var == 'qf' or plot_var == 'pfjrf':
                    plot_ax.plot(np.arange(0,101), plot_data[method][speed][plot_var] / body_weight_factor,
                                 label=' '.join(method.split('_')).title(),
                                 lw=1.5, ls='-', c=plot_col[method], zorder=3)
                # Otherwise just plot raw values
                else:
                    plot_ax.plot(np.arange(0, 101), plot_data[method][speed][plot_var],
                                 label=' '.join(method.split('_')).title(),
                                 lw=1.5, ls='-', c=plot_col[method], zorder=3)

        # Set x-axis parameters
        # Only needs to be done on first axis as they are shared
        ax[0].set_xlim((0,100))
        ...
        # All axes
        [ax[ii].set_xlabel(
            '0-100% Gait Cycle', fontsize=10, fontweight='bold', labelpad=10) for ii in range(len(ax))]

        # Set y-axis parameters
        # Only needs to be done on first axis as they are shared
        ax[0].yaxis.set_major_formatter(FormatStrFormatter('%.0f'))
        if plot_var == 'qf':
            ax[0].set_ylabel('Quadriceps Force (BW)', fontsize=10, fontweight='bold', labelpad=10)
        elif plot_var == 'pfjrf':
            ax[0].set_ylabel('PFJRF (BW)', fontsize=10, fontweight='bold', labelpad=10)
        elif plot_var == 'pfjs':
            ax[0].set_ylabel('PFJS (MPa)', fontsize=10, fontweight='bold', labelpad=10)

        # Add legend on last axis
        ax[-1].legend()
