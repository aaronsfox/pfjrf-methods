...
def run_ik(participant_id, speed):
    ...
    # Filter IK data
    # Uses 10Hz low pass filter as in original paper
    ...
    ik_model.initSystem()
    # Add table operators
    table_proc.append(osim.TabOpLowPassFilter(10))
    ...
    osim.STOFileAdapter().write(ik_table,
                                os.path.join(participant_id, 'ik', speed,
                                             f'{participant_id}_{speed}_ik_complex_filt.mot'))
    ...
    ik_data = osim.TimeSeriesTable(
        os.path.join(participant_id, 'ik', speed, f'{participant_id}_{speed}_ik_complex_filt.mot'))
    ...
    osim.STOFileAdapter().write(ik_data,
                                os.path.join(participant_id, 'ik', speed,
                                             f'{participant_id}_{speed}_ik_simple_filt.mot'))
