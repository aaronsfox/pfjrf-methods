...
participant_list = [
    ii for ii in os.listdir(
        os.path.join('..', '..', 'data', 'Loone2025')
    ) if os.path.isdir(os.path.join('..', '..', 'data', 'Loone2025', ii))]
...
# -------------------------------------------------------------------------
def create_sbatch(participant, condition):

    # Define the sbatch directives
    job_name = f'simulations_{participant}_{condition}'
    ntasks = 1
    mem_per_cpu = '8G'
    cpus_per_task = 12
    timeout = '12:00:00'
    output_file = f'simulations_{participant}_{condition}.log'
...
python run_simulations.py -p {participant} -c {condition}"""

    # Define the filename for the .sh script
    filename = f'simulations_{participant}_{condition}.sh'
    ...
    # Print confirmation
    print(f'{"*" * 10} Created sbatch script for {participant} {condition} {"*" * 10}')
...
python run_simulations.py -p {participant} -c SRRun"""
